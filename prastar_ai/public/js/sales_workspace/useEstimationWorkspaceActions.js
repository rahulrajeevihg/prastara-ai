export function useEstimationWorkspaceActions({
	...
	async function loadDetails() {
		try {
			const res = await window.frappe.call(
				'prastar_ai.api.opportunities.get_opportunity_details',
	...
	async function openItemDetail(item, options = {}) {
		...
		try {
			const res = await window.frappe.call(
				'prastar_ai.api.ai_service.get_estimation_item_pricing_detail',
				{
	...
	async function loadSourceFiles() {
		...
		try {
			const res = await window.frappe.call(
				'prastar_ai.api.ai_service.get_estimation_source_files',
				{ estimation_name: estimationName.value }
			);
	...
	async function startAIAnalysis() {
		...
		try {
			...
			const res = await window.frappe.call(
				'prastar_ai.api.ai_service.process_estimation',
				{
	...
	async function loadMockupsAction() {
		...
		try {
			const res = await window.frappe.call(
				'prastar_ai.api.ai_service.get_estimation_mockups',
				{ estimation_name: estimationName.value }
			);
	...
	async function generateMockups() {
		...
		try {
			...
			const res = await window.frappe.call(
				'prastar_ai.api.ai_service.generate_estimation_mockups',
				{
					estimation_name: estimationName.value,
	...
	async function saveEstimation(options = {}) {
		...
		try {
			...
			await window.frappe.call(
				'prastar_ai.api.opportunities.update_estimation_items',
				{
					estimation_name: estimationName.value,
	...
	async function approveAndConvert(openQuotation) {
		...
		try {
			...
			const res = await window.frappe.call({
				method: 'prastar_ai.api.opportunities.convert_to_quotation',
	...
	async function generateCostBreakdown(options = {}) {
		...
		try {
			const res = await window.frappe.call(
				'prastar_ai.api.ai_service.generate_estimation_cost_breakdown',
				{
					estimation_name: estimationName.value,
					refresh: refresh ? 1 : 0,
				}
	...
	async function loadCommercialReview(options = {}) {
		...
		try {
			const res = await window.frappe.call(
				'prastar_ai.api.ai_service.get_estimation_commercial_review',
	...
	async function loadDrawingTakeoff(options = {}) {
		...
		try {
			const res = await window.frappe.call(
				'prastar_ai.api.ai_service.get_estimation_drawing_takeoff',
				{ estimation_name: estimationName.value, refresh: refresh ? 1 : 0 }
			);
			if (estimation.value) {
	...
	async function restoreVersion(versionName) {
		...
		try {
			await window.frappe.call(
				'prastar_ai.api.opportunities.restore_estimation_version',
	...
	async function refreshCostTemplates() {
		const res = await window.frappe.call('prastar_ai.api.opportunities.get_cost_templates');
	...
	async function saveAsTemplate(templateName, notes = '') {
		...
		try {
			await window.frappe.call(
				'prastar_ai.api.opportunities.save_estimation_as_template',
	...
	async function applyTemplate(templateName, mergeMode = 'append') {
		...
		try {
			await window.frappe.call(
				'prastar_ai.api.opportunities.apply_cost_template',
